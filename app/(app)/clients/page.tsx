            ...
            <div style={{ borderTop: "1px solid #f1f5f9", background: "#fafafa", padding: "14px 16px 18px" }}>
              ...
              {isLoadingSoum ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[1, 2].map(j => (
                    <div key={j} className="sk" style={{ height: 56, borderRadius: 10, background: "white", border: "1px solid #ededeb" }} />
                  ))}
                </div>
              ) : soumissions.length === 0 ? (
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: "white", borderRadius: 10, border: "1px solid #f1f5f9" }}>
                  <Ic d={I.file} z={16} s="#d1d5db" />
                  <p style={{ fontSize: 13, color: "#9ca3af", fontStyle: "italic" }}>Aucune soumission pour ce client.</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {soumissions.map(s => (
                    <Link key={s.id} href={`/soumissions/${s.id}`} onClick={e => e.stopPropagation()} style={{ textDecoration: "none" }}>
                      <motion.div
                        whileTap={{ scale: 0.98 }}
                        style={{
                      ...
                      >
                        {/* Left accent */}
                        <div style={{ width: 3, alignSelf: "stretch", borderRadius: 3, background: ST[s.statut].accentBar, flexShrink: 0 }} />

                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 13, fontWeight: 600, color: "#111827", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {s.titre_projet}
                          </p>
                          <p style={{ fontSize: 10.5, color: "#9ca3af", marginTop: 2, fontFamily: "ui-monospace, monospace", letterSpacing: "0.02em" }}>
