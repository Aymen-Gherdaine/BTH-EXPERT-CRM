...
export default function UtilisateursClient({
  ...
  // Données rendues côté serveur (SSR) → seed immédiat, pas de spinner ni de
  // fetch au montage (revalidation silencieuse en fond ci-dessous).
  const [users,          setUsers]          = useState<UserProfile[]>(initialUsers);
  const [loading,        setLoading]        = useState(false);
  const [currentUserId]                     = useState<string | null>(initialCurrentUserId);
  ...
  // ── Bootstrap ──────────────────────────────────────────────────────────────
  useEffect(() => {
    // Les données SSR sont déjà affichées → revalidation silencieuse en fond.
