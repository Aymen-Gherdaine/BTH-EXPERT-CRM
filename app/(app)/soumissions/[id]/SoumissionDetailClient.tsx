...
export default function SoumissionDetailClient({
  id,
  initialSoumission,
  initialRole,
}: {
  id: string;
  initialSoumission: Soumission;
  ...
  // Tout est rendu côté serveur (SSR) → seed immédiat, aucun spinner ni
  // aller-retour au montage. Le « not found » est géré côté serveur (redirect).
  const [soumission, setSoumission] = useState<Soumission | null>(initialSoumission);
  const [client] = useState<Client | null>(initialSoumission.client ?? null);
  const [lignes] = useState<LigneBudget[]>(initialSoumission.lignes_budget ?? []);
  const [contexte] = useState<SoumissionAIContent | null>(() => {
    if (initialSoumission.contexte_genere) {
      try { return JSON.parse(initialSoumission.contexte_genere) as SoumissionAIContent; } catch {}
    }
    return null;
  });
  ...
  const [role] = useState<UserRole | null>(initialRole);

  // Mode duplication (?duplicate=1) : crée une copie puis redirige. Déclenché
  // une seule fois au montage (garde-fou contre le double-appel StrictMode).
  const didDuplicate = useRef(false);
  useEffect(() => {
    if (isDuplicate && !didDuplicate.current && soumission && client) {
      didDuplicate.current = true;
      handleDuplicate(soumission, client, lignes);
    }
