...
export default function ProfilPageClient({ initialUser }: { initialUser: User | null }) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [user, setUser] = useState<User | null>(initialUser);
  const [avatarUrl, setAvatarUrl] = useState<string | null>(initialUser?.user_metadata?.avatar_url ?? null);
  const [fullName, setFullName] = useState(initialUser ? getDisplayName(initialUser) : "");
